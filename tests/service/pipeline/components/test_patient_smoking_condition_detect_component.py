...
class Icd10SmokingPatternDetectionTest(TestCase):
    ...
    def test_1_run_should_return_bool(self):
        ...
        smoking_pattern_detect_comp = PatientSmokingConditionDetectionComponent(dummy_model.nlp_process)
        dummy_model.set_doc_entity_polarity(polarity=True)
    ...
    def test_2_run_should_return_bool(self):
        dummy_model = DummySciSpacyModel()
        smoking_pattern_detect_comp = PatientSmokingConditionDetectionComponent(dummy_model.nlp_process)
        dummy_model.set_doc_entity_polarity(polarity=False)
        ...
        _smoker: [PatientSmokingCondition] = smoking_pattern_detect_comp.run({"text": text1})
        assert _smoker[0].isSmoker == 1
