...
class TestSpanDiscovery(TestCase):
    ...
    def test__get_icd_10_codes_with_relevant_spans__should_return_correct_response__given_correct_input_setone(self,):
        ...
        assert updated_token_dict["broad"].pos_list[0].start == 115
        assert updated_token_dict["broad"].pos_list[0].end == 120
    ...
    def test__get_icd_10_codes_with_relevant_spans_should_return_correct_response__give_correct_input_settwo(self):
        ...
        assert updated_token_dict["diarrhea"].pos_list[0].start == 407
        assert updated_token_dict["diarrhea"].pos_list[0].end == 415
    ...
    def test__get_icd_10_codes_with_relevant_spans_should_return_correct_response__given_correct_input_setthree(self):
        ...
        assert updated_token_dict["swolling"].pos_list[0].start == 202
        assert updated_token_dict["swolling"].pos_list[0].end == 210
    ...
    def test__get_icd_10_codes_with_relevant_spans_should_return_correct_response__given_correct_input_setfour(self):
        ...
        updated_token_dict, _ = self.text_span_discovery_tool.generate_metainfo_for_changed_text(updated_token_dict, new_ts)
        (span_info, root) = self.text_span_discovery_tool.get_start_end_pos_span(updated_token_dict, "swell", 0)
        assert span_info == 199
        assert root == "noswolling"
        assert updated_token_dict["swolling"].parent_token == "noswolling"
        assert updated_token_dict["swolling"].pos_tracking[202] == 201
        assert updated_token_dict["swolling"].pos_list[0].start == 202
        assert updated_token_dict["swolling"].pos_list[0].end == 210
        ...
        assert updated_token_dict["swolling"].parent_token == "noswolling"
        assert updated_token_dict["swolling"].pos_tracking[202] == 201
        assert updated_token_dict["swell"].parent_token == "swolling"
        assert updated_token_dict["swell"].pos_tracking[202] == 202
        assert updated_token_dict["hard"].pos_tracking[217] == 217